...
- 直通模式加载 `raw.json`、`ebpf.json` 与 `08_services.json`，绝不能再带 `-C confdir`——sing-box 合并多份配置是数组追加而非覆盖，托管配置会和用户配置叠加出重复的 `inbounds`、`dns.servers` 和出站标签，核心起不来且报错指向用户配置。
...
- 直通模式仍要生成 eBPF 入站——它是模块在 Android 上唯一的抓取入口，用户配置删掉 `tun` 后如果这里也不提供入站，核心会正常 ready 但一个连接都不会被代理，日志里没有任何错误。
