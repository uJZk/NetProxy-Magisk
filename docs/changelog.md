...
## 版本 8.1.1.2（2026-09-04）

8.1.1.2 修复命令超时预算不足导致的误报失败。

### 修复

- `service restart`、`reload` 与 `toggle` 此前沿用 30 秒默认时限，而它们内部要停止核心、生成配置、运行 sing-box 检查并等待控制接口就绪，仅等待就绪一项就有 30 秒预算。核心仍在启动时命令就会返回 `command.timeout`，容易被误判为启动失败。现在与 `service start` 一致取 120 秒。
- `config apply` 在核心运行时会触发一次完整重载（生成配置 → sing-box 检查 → 重启 → 等待就绪），配置中的远程规则集还需要重新下载，同样会超过默认时限。Android 管理器的「启动设置」等开关走的正是这条路径，因此会显示保存失败，而磁盘上的设置其实可能已经写入。`config apply`、`validate`、`check` 与 `config raw update` 现在取 120 秒；`list`、`read`、`raw show` 等只读操作保持 30 秒。

### 说明

- 本次只调整命令行的超时预算，服务状态机、配置格式与磁盘布局均未改动。

* * *

## 版本 8.1.1.1（2026-09-04）

8.1.1.1 支持直接使用 sing-box 完整配置：既可以从中导入节点，也可以让模块按周期下载并直接运行整份配置。
